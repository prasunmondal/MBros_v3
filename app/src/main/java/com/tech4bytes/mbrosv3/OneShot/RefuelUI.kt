...
class RefuelUI {
    ...
    companion object {
        ...
        fun initiallizeRefuelUI(activity: Activity, uiContainer: LinearLayout) {
            ...
            val refuelingKmElementPart1 = uiContainer.findViewById<EditText>(R.id.one_shot_delivery_refueling_km_part1)
            val refuelingKmElementPart2 = uiContainer.findViewById<EditText>(R.id.one_shot_delivery_refueling_km_part2)
            meteredFuelKms = MeteredNumbers(refuelingKmElementPart1, refuelingKmElementPart2, 3)
            meteredFuelKms.setListeners {updateRefuelingUIDetails(activity, uiContainer)}
            ...
            activity.runOnUiThread {
                ...
                meteredFuelKms.setNumber(activity, NumberUtils.getIntOrZero(RefuelingUtils.getPreviousRefuelingKM()), true)
            }

            refuelQtyElement.doOnTextChanged { text, start, before, count ->
                updateRefuelingUIDetails(activity, uiContainer)
                updateFuelRate(uiContainer)
            }
...
//        refuelAmountElement = findViewById(R.id.osd_refuel_amount)
        ...
        fun updateRefuelingUIDetails(activity: Activity, uiContainer: LinearLayout) {
            if (meteredFuelKms.getNumber() != null) {
                val mileageLabel = uiContainer.findViewById<TextView>(R.id.one_shot_delivery_refueling_mileage)
                ...
                if (RefuelingUtils.getKmDifferenceForRefueling(meteredFuelKms.getNumber()!!) > 0) {
                    ...
                    meteredKm.setNumber(activity, refuelingKm + addKmToFuelKmToGetFinalKm, false)
                }
            }
        }
        ...
        fun updateFuelRate(uiContainer: LinearLayout) {
            val oilRateLabel = uiContainer.findViewById<TextView>(R.id.one_shot_delivery_refueling_oil_rate_per_litre)
            ...
            if(fuelRateInInt < lowerRateLimit || fuelRateInInt > upperRateLimit) {
                oilRateLabel.setBackgroundColor(ContextCompat.getColor(AppContexts.get(), R.color.osd_fuel_rate_not_matching))
                oilRateLabel.setTextColor(ContextCompat.getColor(AppContexts.get(), R.color.white))
            } else {

                oilRateLabel.setBackgroundColor(0x00000000)
                oilRateLabel.setTextColor(ContextCompat.getColor(AppContexts.get(), R.color.osd_fuel_non_interactive_ok_text_color))
            }
        }
        ...
        fun initializeFinalKm(activity: Activity, uiFinalKmContainer: LinearLayout) {
            ...
            val finalKmElementFirstPart = uiFinalKmContainer.findViewById<EditText>(R.id.one_shot_delivery_trip_end_km_first_part)
            val finalKmElementSecondPart = uiFinalKmContainer.findViewById<EditText>(R.id.one_shot_delivery_trip_end_km_second_part)
            meteredKm = MeteredNumbers(finalKmElementFirstPart, finalKmElementSecondPart, 3)

            val salaryDivisionElement = uiFinalKmContainer.findViewById<TextView>(R.id.osd_salary_division)

            val vehiclePrevKm: Int = NumberUtils.getIntOrZero(SingleAttributedDataUtils.getRecords().vehicle_finalKm)

            meteredKm.setNumber(activity, vehiclePrevKm, true)
            meteredKm.setListeners {
                updateKmRelatedCosts(activity, uiFinalKmContainer)
            }
        ...
        fun gatherFuelData(uiContainer: LinearLayout) {
            val obj = SingleAttributedDataUtils.getRecords()
            val didRefuelElement = uiContainer.findViewById<Switch>(R.id.one_shot_delivery_did_refuel)
            obj.refueling_km = ""
